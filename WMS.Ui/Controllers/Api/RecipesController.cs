   ...
   public class RecipesController : ControllerBase
   {
      ...
      public async Task<IActionResult> UpdateHitsAsync(int id)
      {
         try
         {
            ...
            await modifyRecipesCommand.UpdateAsync(recipeDto).ConfigureAwait(false);
            return Accepted();
         }
         catch (Exception)
         {
      ...
      public async Task<IActionResult> UpdateRatingAsync(int id, [FromBody] string value)
      {
         try
         {
            ...
            // get record from db
            var getRecipesQuery = _queryFactory.CreateRecipesQuery();
            var recipe = await getRecipesQuery.ExecuteAsync(id).ConfigureAwait(false);
            ...
            if (rating == null)
            {
               rating = new Business.Recipe.Dto.RatingDto
               {
                  RecipeId = recipe.Id,
                  TotalValue = 0,
                  TotalVotes = 0,
            ...
            if (newRating)
               await modifyRatingsCommand.AddAsync(rating).ConfigureAwait(false);
            else
               await modifyRatingsCommand.UpdateAsync(rating).ConfigureAwait(false);
