...
namespace WMS.Ui.Mvc6.Controllers
{
    ...
    public class AccountController : BaseController
    {
        ...
        public AccountController(IWebHostEnvironment environment, IConfiguration configuration,
            ...
            IOptions<AppSettings> appSettings, RWD.Toolbox.SMTP.IEmailAgent emailAgent) :
        ...
        public async Task<IActionResult> Login(LoginViewModel model, Uri? returnUrl)
        {
            ViewData["Title"] = "Login";
            ViewData["ReturnUrl"] = returnUrl;
            if (ModelState.IsValid)
            {
                ...
                if (user != null)
                {
                    ...
                    {
                        var availableNext = DateTime.Now.AddDays(1).ToString("g", CultureInfo.CurrentCulture);
                        if (user.LockoutEnd.HasValue)
                            availableNext = user.LockoutEnd.Value.ToLocalTime().ToString("g", CultureInfo.CurrentCulture);
                        ModelState.AddModelError("", string.Format(CultureInfo.CurrentCulture, $"Due to multiple failed login attempts, your account has been locked out until {availableNext}"));
                    }
                    else
                    {
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Register(RegisterViewModel model, Uri? returnUrl)
        {
            ViewData["Title"] = "Register";
            ViewData["ReturnUrl"] = returnUrl;
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    var confirmationToken = await UserManagerAgent.GenerateEmailConfirmationTokenAsync(user).ConfigureAwait(false);
                    ...
                    var emailName = string.IsNullOrWhiteSpace(user.FirstName) ? user.UserName ?? string.Empty : $"{user.FirstName} {user.LastName}".Trim();
        ...
        public async Task<IActionResult> ResetPassword(ResetPasswordViewModel model)
        {
            ...
            if (result.Succeeded)
            {
                Success("Your password has been reset.  Please log in.", true);
                return View("Login");
            }
            else
            {
                Danger("Your password could not be reset.  Please request a new password again.");
