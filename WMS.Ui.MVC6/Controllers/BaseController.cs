...
namespace WMS.Ui.Mvc6.Controllers
{
    ...
    public class BaseController : Controller
    {
        ...
        private void AddAlert(string alertStyle, string message, bool dismissable)
        {
            ...
            {
                var existingAlerts = (List<Alert>?)TempData[Alert.TempDataKey];
        ...
        public async Task<byte[]> ResizeImage(byte[] buffer, int? maxWidth, int? maxHeight)
        {
            ...
            {
                // TODO Log Verify API key and account limit.
                var validAccount = await Tinify.Validate().ConfigureAwait(false);
                var compressionsThisMonth = Tinify.CompressionCount;                
                var accountMsg = accountEx.Message;                
            }
            catch (Exception)
            {
                throw;
