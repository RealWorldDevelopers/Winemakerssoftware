...
{
   public class GetBatchVolumeUOM : IQuery<IUnitOfMeasure>
   {
      ...
      public GetBatchVolumeUOM(WMSContext dbContext, IMapper mapper)
      {
      ...
      {
         var UnitOfMeasure = _dbContext.UnitsOfMeasure.Where(uom => uom.Subset == Common.Subsets.Volume.Batch).ToList();
      ...
      public IUnitOfMeasure Execute(int id)
      {
         var category = _dbContext.UnitsOfMeasure
            .Where(uom => uom.Subset == Common.Subsets.Volume.Batch)
